...
class Messenger:
    ...
    SMTP_PASSWORD = "AuroraServer##"
    EMAIL_FROM = "auroracoin.wallet@gmail.com"
    EMAIL_SUBJECT = "+Aurtip response"
    def __init__(self, db, bbmech, messages=None, api=None):
        if messages == None:
            pass
        else:
            ...
            for message in self.messages:
                ...
                if count == num_messages:
                    ...
                    if message[2] == "public":
                        ...
                        print message_text
                        if api.api_type == "bland":
                            bbmech.post(api, 0, thread_id[1], message_text)
                        elif api.api_type == "phpbb":
                            thread_id = message[0].split("-", 3)
                            bbmech.post(api, thread_id[0], thread_id[1], message_text)
